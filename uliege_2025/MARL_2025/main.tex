...
\begin{frame}{MARL: Framework}
    Partially observable Stochastic Game (also referred to as Markov Game) $[n, \mathcal{S}, O, \mathcal{Z}, \mathcal{U}, r, P, \gamma]$:
    \vfill
    \begin{itemize}
    ...
    \end{itemize}{}
\end{frame}
\begin{frame}{MARL hierarchy}
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{POSG_hierarchy.pdf}
    \caption{POSG hierachy.}
\end{figure}
\blfootnote{Stefano V. Albrecht, Filippos Christianos, and Lukas Schäfer. Multi-Agent Reinforcement Learning: Foundations and Modern Approaches. MIT Press, 2024. }
\end{frame}
...
\begin{subfigure}[b]{0.33\textwidth}
        \centering
        \includegraphics[width=.95\textwidth]{MATE.png}
        \caption{MATE.}
\end{subfigure}
\end{figure}
\end{frame}
...
\begin{frame}{Cooperation: environment example}
...
\begin{frame}{Cooperation: QMIX architecture}
\begin{figure}
    \centering
    \includegraphics[width=.9\textwidth]{qmix_archi.png}
...
\begin{frame}{Cooperation: QMIX results}
\begin{figure}
    \centering
...
\begin{frame}{Cooperation: QVMix results}
\begin{figure}
    \centering
...
\begin{frame}{Cooperation: QVMix overestimation bias}
\begin{figure}
    \centering
...
\begin{frame}{Cooperation: Policy-based methods in CTDE}
...
\begin{frame}{Cooperation: centralised critic}
Problem: How to benefit from centralised information such as $s_t$?
\vfill
Solutions:
\begin{itemize}
...
Problems solved addressed in COMA:
\begin{itemize}
    \item Based on global rewards $r_t$.
    \item The centralised critic does not solve the credit assignment problem.
\end{itemize}
\blfootnote{Foerster, J., Farquhar, G., Afouras, T., Nardelli, N.,  Whiteson, S. (2018). Counterfactual multi-agent policy gradients. }
\end{frame}

\begin{frame}{Cooperation: MAPPO}
Multi-Agent Proximal Policy Optimization (MAPPO):

\begin{align*}
    \mathcal{L}(\theta)\hspace{-3pt} = & [\frac{1}{Bn} \sum\limits_{i = 1}^{B} \sum\limits_{k = 1}^n \text{min}( r_{\theta, i}^{(k)}A_i^{(k)}, \text{clip}( r_{\theta, i}^{(k)}, 1 - \epsilon, 1 + \epsilon) A_i^{(k)})] \\
    & + \sigma \frac{1}{Bn} \sum\limits_{i = 1}^{B} \sum\limits_{k = 1}^n \hspace{-2pt} S[\pi_{\theta}(o_i^{(k)}))]
\end{align*}
\begin{itemize}
\item $r_{\theta, i}^{(k)} = \frac{\pi_{\theta}(a_i^{(k)}|o_i^{(k)})}{\pi_{\theta_{old}}(a_i^{(k)}|o_i^{(k)})}$.,
\item $A_i^{(k)}$ is computed using the GAE method and a \textbf{centralised} critic,
\item $S$ is the policy entropy,
\item $\sigma$ is the entropy coefficient hyperparameter.
\end{itemize}

\blfootnote{Yu, C., Velu, A., Vinitsky, E., Gao, J., Wang, Y., Bayen, A. \& Wu, Y. (2022). The surprising effectiveness of ppo in cooperative multi-agent games}
\end{frame}
...
\begin{table}
\centering  
\resizebox{.5\textwidth}{!}{\begin{tabular}{ccccc}
\toprule 
Map              & MAPPO\scriptsize{(FP)}  & MAPPO\scriptsize{(AS)}  & IPPO       & QMix       \\
\midrule
\scriptsize{2m vs\_1z}        & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} & \textbf{95.3}\scriptsize{(5.2)}  \\
\scriptsize{3m}               & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(1.5)} & \textbf{100.0}\scriptsize{(0.0)} & 96.9\scriptsize{(1.3)}  \\
\scriptsize{2svs1sc}          & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(1.5)} & 96.9\scriptsize{(2.9)}  \\
\scriptsize{2s3z}             & \textbf{100.0}\scriptsize{(0.7)} & \textbf{100.0}\scriptsize{(1.5)} & \textbf{100.0}\scriptsize{(0.0)} & 95.3\scriptsize{(2.5)}  \\
\scriptsize{3svs3z}           & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} & \textbf{96.9}\scriptsize{(12.5)} \\
\scriptsize{3svs4z}           & \textbf{100.0}\scriptsize{(1.3)} & \textbf{98.4}\scriptsize{(1.6)}  & \textbf{99.2}\scriptsize{(1.5)} & \textbf{97.7}\scriptsize{(1.7)}  \\
\scriptsize{so many baneling}  & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.7)} & \textbf{100.0}\scriptsize{(1.5)} & 96.9\scriptsize{(2.3)}  \\
\scriptsize{8m}               & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.7)} & 97.7\scriptsize{(1.9)}  \\
\scriptsize{MMM}              & \textbf{96.9}\scriptsize{(0.6)}  & 93.8\scriptsize{(1.5)}  & \textbf{96.9}\scriptsize{(0.0)} & \textbf{95.3}\scriptsize{(2.5)}  \\
\scriptsize{1c3s5z}           & \textbf{100.0}\scriptsize{(0.0)} & 96.9\scriptsize{(2.6)}  & \textbf{100.0}\scriptsize{(0.0)} & 96.1\scriptsize{(1.7)}  \\
\scriptsize{bane vs bane}     & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} \\
\scriptsize{3svs5z}           & \textbf{100.0}\scriptsize{(0.6)} & \textbf{99.2}\scriptsize{(1.4)}  & \textbf{100.0}\scriptsize{(0.0)} & \textbf{98.4}\scriptsize{(2.4)}  \\
\scriptsize{2cvs64zg}         & \textbf{100.0}\scriptsize{(0.0)} & \textbf{100.0}\scriptsize{(0.0)} & 98.4\scriptsize{(1.3)}  & 92.2\scriptsize{(4.0)}  \\
\scriptsize{8mvs9m}           & \textbf{96.9}\scriptsize{(0.6)} & \textbf{96.9}\scriptsize{(0.6)}  & \textbf{96.9}\scriptsize{(0.7)} & 92.2\scriptsize{(2.0)}  \\
\scriptsize{25m}              & \textbf{100.0}\scriptsize{(1.5)} & \textbf{100.0}\scriptsize{(4.0)} & \textbf{100.0}\scriptsize{(0.0)} & 85.9\scriptsize{(7.1)}  \\
\scriptsize{5mvs6m}           & \textbf{89.1}\scriptsize{(2.5)}  & \textbf{88.3}\scriptsize{(1.2)}  & \textbf{87.5}\scriptsize{(2.3)} & 75.8\scriptsize{(3.7)}  \\
\scriptsize{3s5z}             & \textbf{96.9}\scriptsize{(0.7)}  & \textbf{96.9}\scriptsize{(1.9)}  & \textbf{96.9}\scriptsize{(1.5)} & 88.3\scriptsize{(2.9)}  \\
\scriptsize{10mvs11m}         & \textbf{96.9}\scriptsize{(4.8)}  & \textbf{96.9}\scriptsize{(1.2)}  & \textbf{93.0}\scriptsize{(7.4)} & \textbf{95.3}\scriptsize{(1.0)}  \\
\scriptsize{MMM2}             & \textbf{90.6}\scriptsize{(2.8)}  & \textbf{87.5}\scriptsize{(5.1)}  & \textbf{86.7}\scriptsize{(7.3)} & \textbf{87.5}\scriptsize{(2.6)}  \\
\scriptsize{3s5zvs3s6z}       & \textbf{84.4}\scriptsize{(34.0)} & 63.3\scriptsize{(19.2)} & \textbf{82.8}\scriptsize{(19.1)} & \textbf{82.8}\scriptsize{(5.3)}  \\
\scriptsize{27mvs30m}         & \textbf{93.8}\scriptsize{(2.4)}  & 85.9\scriptsize{(3.8)}  & 69.5\scriptsize{(11.8)} & 39.1\scriptsize{(9.8)}  \\
\scriptsize{6hvs8z}           & \textbf{88.3}\scriptsize{(3.7)}  & \textbf{85.9}\scriptsize{(30.9)} & \textbf{84.4}\scriptsize{(33.3)} & 9.4\scriptsize{(2.0)}   \\
\scriptsize{corridor}         & \textbf{100.0}\scriptsize{(1.2)} & \textbf{98.4}\scriptsize{(0.8)}  & \textbf{98.4}\scriptsize{(3.1)} & 84.4\scriptsize{(2.5)}  \\
\bottomrule
\end{tabular}}
\centering
\caption{Median evaluation win rate and standard deviation on all the SMAC maps for different methods.
All values within 1 standard deviation of the maximum are bolded.
}
\end{table}
...
% \begin{frame}{Cooperation: COMA architecture}
% \begin{figure}
%     \centering
...
% \begin{frame}{Cooperation: COMA results}
% \begin{figure}
%     \centering
...
% \begin{frame}{Cooperation: COMA vs QMIX}
% \begin{figure}
%     \centering
...
% \begin{frame}{Cooperation: IMP variations}
% \begin{figure}
...
\begin{frame}{Cooperation: IMP performance} %TODO
...
First methods, RIAL and DIAL: agents learn $Q^a(o^a_t, m^{-a}_{t-1}, u_t, h_{t-1})$.
\vfill
Successor methods tackle other challenges with new frameworks.
\vfill
...
\end{itemize}
\blfootnote{RIAL and DIAL: Foerster, J. N., Assael, Y. M., De Freitas, N., Whiteson, S. (2016). Learning to communicate with deep multi-agent reinforcement learning.}
\end{frame}
...
Nash equilibrium:
\begin{itemize}
    \item Extending the concept of mutual best responses to general-sum games with two or more agents.
    \item This equilibrium is achieved when no agent benefits from changing its current policy.
    \item A joint policy $\mathbf{\pi}$ is a Nash equilibrium if $\forall a_i$, any $\pi'_i$ is such that $$G^i(\pi'_i , \mathbf{\pi^{-i}}) \le G^i(\mathbf{\pi})$$.
\end{itemize}
\vfill
\pause
This is often a solution people are looking for, \textbf{BUT}:
\begin{itemize}
    \item Equilibrium does not mean maximized reward (sub-optimality).
    \item Multiple equilibria exist (non-uniqueness).
    \item Deterministic policies may not achieve Nash equilibrium.
    \item Hard to find and hard to verify.
\end{itemize}
\end{frame}
...
\begin{frame}{Competition: Minimax-Q}
...
\begin{itemize}
    ...
    \vfill
\end{itemize}
The neural network $f_\theta(s)$ is trained in self-play: the agent plays against itself.
\end{frame}
...
\end{enumerate}
\vfill
\textbf{\textcolor{red}{Note that the notation has changed: an action is represented by $a$.}}
\end{frame}

\begin{frame}{Competition: AlphaGo Zero self-play}
\begin{figure}
    \centering
...
\begin{frame}{Competition: AlphaGo Zero MCTS}
\begin{figure}
    \centering
...
\begin{frame}{Competition: AlphaGo Zero training}
...
\begin{frame}{Competition: AlphaGo Zero numbers}
...
\begin{frame}{Competition: Two team markov game}
